...
Global $sTesseractPath = @ProgramFilesDir & "\tesseract\tesseract.exe"
...
Func _TesseractPathSet($sPath)
	$sTesseractPath = $sPath
...
func _TesseractScreenCapture($get_last_capture = 0, $delimiter = "", $cleanup = 1, $scale = 2, $left_indent = 0, $top_indent = 0, $right_indent = 0, $bottom_indent = 0, $show_capture = 0)
	...
	ShellExecuteWait($sTesseractPath, $capture_filename & " " & $ocr_filename)
...
;
; ;==========================================================================================
func _TesseractWinCapture($win_title, $win_text = "", $get_last_capture = 0, $delimiter = "", $cleanup = 1, $scale = 2, $sLang = 'eng', $iPSM = 0, $left_indent = 0, $top_indent = 0, $right_indent = 0, $bottom_indent = 0, $show_capture = 0)
	...
	ShellExecuteWait($sTesseractPath, $capture_filename & " " & $ocr_filename & ' -l ' & $sLang & ' --psm ' & $iPSM)
	...
	if $show_capture = 1 Then
		...
		_SetImage($hPic, $capture_filename)
...
Func CaptureToTIFF($win_title = "", $win_text = "", $ctrl_id = "", $sOutImage = "", $scale = 1, $left_indent = 0, $top_indent = 0, $right_indent = 0, $bottom_indent = 0)
	...
	if IsHWnd($hwnd2) Then
		...
		$hBitmap2 = _ScreenCapture_CaptureWnd('', $hwnd2, $left_indent, $top_indent, $right_indent, $bottom_indent, False)

		Local $pos = [0, 0, $right_indent - $left_indent, $bottom_indent - $top_indent]
	Else
	...
	$hImage2 = _GDIPlus_ImageScale($hImage2, $scale, $scale, $GDIP_INTERPOLATIONMODE_BICUBIC)
 ...
 ;    $hBMP = _WinAPI_CreateCompatibleBitmap($hDC, ($pos[2] * $scale) - ($right_indent * $scale), ($pos[3] * $scale) - ($bottom_indent * $scale))
	...
	_GDIPlus_ImageSaveToFileEx($hImage2, $sOutImage, $CLSID, $pParams)
